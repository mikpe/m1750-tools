...
#endif
//- m1750	  funexp = force_reg (Pmode, funexp);
     ...
     int is_const;
{
  ...
  if (HAVE_call && HAVE_call_value)
    {
      /* m1750 patch */
      if (fndecl != NULL && far_call_p (fndecl))
        {
          if (valreg)
            emit_call_insn (gen_far_call_value (valreg,
                                            gen_rtx (MEM, HImode, funexp),
                                            stack_size_rtx));
          else
            emit_call_insn (gen_far_call (gen_rtx (MEM, HImode, funexp),
                                      stack_size_rtx));
        }
      else
        {
