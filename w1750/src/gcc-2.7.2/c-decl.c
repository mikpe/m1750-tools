  ...
  if (name)
      ...
      if (t != 0 && duplicate_decls (x, t))
	{
	  ...
	  if (!flag_traditional && TREE_PUBLIC (name)
	    ...
	    {
	      error ("`%s' was declared implicitly `extern' and later `static'",
      ...
      if (b == global_binding_level)
	{
	  ...
	  if (IDENTIFIER_IMPLICIT_DECL (name) != 0
	      ...
	      && ! (TREE_CODE (x) == FUNCTION_DECL
			...
			== integer_type_node)))
	    error ("`%s' was previously implicitly declared to return `int'",
		     IDENTIFIER_POINTER (name));
	  ...
	  if (TREE_PUBLIC (name)
		...
		error ("`%s' was declared `extern' and later `static'",
			 IDENTIFIER_POINTER (name));
	    }
	}
      else
