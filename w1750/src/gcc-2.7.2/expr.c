  ...
  switch (code)
    ...
    case ADDR_EXPR:
	 ...
	 be a MEM corresponding to a stack slot. */
      ...
      /* the 1750 does not support trampolines or address of public function
         if we are compiling for expanded memory */
      if (TREE_CODE (TREE_OPERAND (exp, 0)) == FUNCTION_DECL)
        {
          if (decl_function_context (TREE_OPERAND (exp, 0)) != 0)
            {
              error ("indirect calls to nested functions not supported");
	      return const0_rtx;
            }

          if (!taking_address_ok (TREE_OPERAND (exp, 0)))
            warning ("address incompatible with expanded memory");
        }
