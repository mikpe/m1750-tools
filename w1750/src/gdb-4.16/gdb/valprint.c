...
/* On the 1750, we get 16 bit chars */
typedef unsigned short char1750;
    ...
    GDB_FILE *stream;
{
  ...
  unsigned int nfetch;		/* 16-bit Bytes to fetch / bytes fetched. */
  unsigned int chunksize;	/* Size of each fetch, in bytes. */
  unsigned int bufsize;		/* Size of current fetch buffer. */
  char1750 *buffer = NULL;		/* Dynamically growable fetch buffer. */
  char1750 *bufptr;			/* Pointer to next available byte in buffer. */
  char1750 *limit;			/* First location past end of fetch buffer. */
  ...
  do {
    ...
    if (buffer == NULL)
      {
	buffer = (char1750 *) xmalloc (bufsize * sizeof (char1750));
	bufptr = buffer;
      }
    else
      {
	discard_cleanups (old_chain);
	buffer = (char1750 *) xrealloc (buffer, bufsize * sizeof (char1750));
	bufptr = buffer + bufsize - nfetch;
      }
    old_chain = make_cleanup (free, buffer);

    /* Read as much as we can. */
    nfetch = target_read_memory_partial (addr, bufptr, nfetch * sizeof (char1750), &errcode);
    nfetch /= sizeof (char1750);

#if 0
    /* CCFN, correct for host byte order */
    {
      int i;

      for (i = 0; i < nfetch; i++)
        {
          bufptr [i] = extract_unsigned_integer (&bufptr [i], 2);
        }
    }
#endif
