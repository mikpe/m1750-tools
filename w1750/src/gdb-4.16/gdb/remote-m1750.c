...
//ccfn  target_mourn_inferior ();
}

static void
remote_create_inferior (exec_file, args, env)
     char *exec_file;
     char *args;
     char **env;
{
  ...
  if (args && *args)
    error ("Can't pass arguments to remote M1750");
     ...
     int from_tty;
{
  if (remote_desc == NULL)
    {
      error( "No target system to reset -- use 'target remote' command.");
    }
...
Specify the serial device it is connected to (e.g. /dev/ttya).",  /* to_doc */
  ...
  remote_create_inferior,	/* to_create_inferior */
