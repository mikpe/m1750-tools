...
extract_address (addr, len)
     ...
     int len;
{
  /* CCFN: The 1750 is a word-addressed machine. An address in memory or in a
     register is 16-bits wide, and can address 64K words of memory. The
     expanded memory option use 8 high-significance address bits from
     the SW register to extend addresses to 24 bits.  */

  /* Assume a CORE_ADDR can fit in a LONGEST (for now).  Not sure
     whether we want this to be true eventually.  */
//CCFN  return (unsigned long)(extract_unsigned_integer (addr, len) );
// FIXME: how do we cope with expanded memory?
  return (unsigned long)(extract_unsigned_integer (addr, 2) << 1);
}
     ...
     CORE_ADDR val;
{
  /* Assume a CORE_ADDR can fit in a LONGEST (for now).  Not sure
     whether we want this to be true eventually.  */
  store_unsigned_integer (addr, 2, (LONGEST)val >> 1); // CCFN
...
{
  DOUBLEST
  m1750_extract_floating (PTR addr, int len);


  return m1750_extract_floating (addr, len);
...
store_floating (addr, len, val)
     ...
     DOUBLEST val;
{
  void m1750_store_floating (PTR addr, int len, DOUBLEST val);
...
// CCFN: I've commented this out because I don't know what it's for
#if 0
